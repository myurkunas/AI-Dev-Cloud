...
- **UPDATE (Member 2):** the 6-doc seed has been **replaced** with the vetted
  corpus — 107 chunks from 32 official pages — generated by
  `data/sources/build_corpus.py` from `data/sources/chunks.csv`. The full
  clean → chunk → verify pipeline and the approved-source registry live in
  `data/sources/` (see its README).
